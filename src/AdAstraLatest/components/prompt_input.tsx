...
export function PromptInput({
  ...
  return (
    <div 
      className={cn(
        'bg-[rgba(0,0,0,0.02)] box-border content-stretch flex flex-col items-start justify-between p-[24px] rounded-[24px] w-full relative transition-all duration-300',
        attachments.length > 0 ? 'h-[200px]' : 'h-[135px]',
        disabled && 'opacity-50 pointer-events-none',
        className
      )}
    >
      <div className="absolute border border-[rgba(0,0,0,0.15)] border-solid inset-0 pointer-events-none rounded-[24px]" />
      ...
      <button
        ...
        className="absolute bottom-[10px] right-[10px] size-[42px] hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-[#5250F3] focus:ring-offset-2 rounded-full"
      >
        <div className="absolute aspect-[32/32] left-0 right-0 top-0">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 42 42">
            <circle cx="21" cy="21" fill="#5250F3" r="21" />
          </svg>
        </div>
      ...
      <button
        ...
        disabled={disabled}
        className="absolute left-[24px] bottom-[10px] overflow-clip size-[20px] hover:scale-110 transition-transform disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 rounded"
      >
      ...
      <textarea
        ...
        disabled={disabled}
        className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.4] min-w-full relative shrink-0 text-[14px] text-[rgba(0,0,0,0.85)] w-full h-full bg-transparent border-none outline-none resize-none placeholder:text-[rgba(0,0,0,0.85)] pr-[60px] pb-[40px]"
