...
export function VideoControl({
  isPlaying = false,
  currentTime = 0,
  duration = 100, // default to 100s like file_1 totalTime
  progress = 0,
  onPlayPause,
  onProgressChange,
  onSettingsClick,
  ...
  // ===== Ported interactive behavior from file_1 =====
  ...
  const handleProgressClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const next = Math.min(Math.max((x / rect.width) * 100, 0), 100)
    setInternalProgress(next)
    setInternalCurrentTime((next / 100) * duration)
    onProgressChange?.(next)
  }

  // Drag seek (global listeners during drag)
  ...
  // Play/Pause (internal toggle + callback)
  ...
  useEffect(() => {
    if (internalPlaying && internalProgress < 100) {
      const tick = () => {
        const now = Date.now()
        ...
        setInternalProgress(prev => {
          const next = prev + (dt / (duration || 1)) * 100
          if (next >= 100) {
        ...
        setInternalCurrentTime(prev => Math.min(prev + dt, duration))
    ...
    } else if (!internalPlaying && rafRef.current) {
      cancelAnimationFrame(rafRef.current)
      rafRef.current = null
    }
  }, [internalPlaying, internalProgress, duration])

  // If parent updates starting props (rare), adopt them when not dragging/playing
  useEffect(() => {
    if (!isDragging && !internalPlaying) {
      setInternalProgress(progress)
      setInternalCurrentTime(currentTime)
      setInternalPlaying(isPlaying)
    }
  }, [progress, currentTime, isPlaying, isDragging, internalPlaying])

  const clamp = (n: number, lo = 0, hi = 100) => Math.min(Math.max(n, lo), hi)
  const progressPct = clamp(internalProgress)
  ...
  return (
    <div className={cn('box-border flex flex-col gap-[12px] h-[96px] items-start p-[24px] w-[327px]', className)}>
      {/* Progress Bar */}
      <div className="bg-[rgba(0,0,0,0.5)] h-[8px] relative rounded-[999px] shrink-0 w-full cursor-pointer" onClick={handleProgressClick}>
        <div ref={barRef} className="absolute inset-0" />
        <div
          className={cn('absolute bg-[#5250F3] h-[8px] left-0 rounded-[999px] top-0', !isDragging && 'transition-all duration-150')}
          style={{ width: `${progressPct}%` }}
        />
        {/* Thumb area */}
        <div
          className="absolute w-[50px] h-[50px] top-[-21px] cursor-grab active:cursor-grabbing"
          style={{ left: `calc(${progressPct}% - 25px)` }}
        ...
        >
          <svg className="block w-full h-full" fill="none" viewBox="0 0 50 50">
            <g filter="url(#filter0_d_0_13)">
              ...
              <circle cx="25" cy="25" r="6.5" stroke="rgba(0,0,0,0.15)" strokeWidth="1" />
            </g>
            <defs>
              <filter colorInterpolationFilters="sRGB" filterUnits="userSpaceOnUse" height="48.2" id="filter0_d_0_13" width="48.2" x="0.9" y="0.9">
                ...
                <feMorphology in="SourceAlpha" operator="dilate" radius="5" result="effect1_dropShadow_0_13" />
                <feOffset />
      ...
      {/* Controls */}
      <div className="flex items-center justify-between relative shrink-0 w-full h-[24px]">
        {/* Left Controls */}
        <div className="flex gap-[8px] items-center relative shrink-0">
          <button className="overflow-clip relative shrink-0 size-[24px] hover:opacity-80 transition-opacity" onClick={togglePlayPause}>
            <div className="absolute inset-[12.5%_20.83%]">
              <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 18 22">
                {internalPlaying ? (
                  <g>
                    <rect x="2" y="2" width="5" height="18" fill="black" rx="1" />
                    <rect x="11" y="2" width="5" height="18" fill="black" rx="1" />
                  </g>
                ) : (
                  <path d="M2 2L16 11L2 20V2Z" stroke="black" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" />
                )}
              </svg>
            </div>
          </button>
